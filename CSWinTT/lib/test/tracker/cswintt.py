...
class CSWinTT(BaseTracker):
	...
	def track_multi_state(self, image, info):
		...
		if self.first_track and self.max_confidence > 0.3:  # 첫번째 프레임에서 추적할때 0.5 보다 높으면 template 에 추가함.
		...
		elif not self.first_track and self.max_confidence > 0.3:  # 두번째 프레임에서부터는 점수 상관없이 모두 추가함
			self.focal_max_template.append([self.max_confidence, max_state, z_dict_t, info])
		# elif not self.first_track and 0 < self.max_confidence < 0.5:
		else:
			self.focal_max_template.append([self.max_confidence, max_state, z_dict_t, info])
