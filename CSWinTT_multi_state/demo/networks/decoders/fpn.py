...
from CSWinTT_multi_state.demo.networks.layers.basic import ConvGN


class FPNSegmentationHead3(nn.Module):
    def __init__(self,
                 in_dim,
                 out_dim,
                 decode_intermediate_input=True,
                 hidden_dim=256,
                 shortcut_dims=[256, 512, 1024, 1024],
                 align_corners=True):
        super().__init__()
    ...
    def forward(self, inputs, fusion):
        ...
        x = F.relu_(self.conv_16x(self.adapter_16x(inputs[-2]) + x))

        x = F.interpolate(x,
                          size=inputs[-3].size()[-2:],
                          mode="bilinear",
                          align_corners=self.align_corners)
        x = F.relu_(self.conv_8x(self.adapter_8x(inputs[-3]) + x))

        x = F.interpolate(x,
                          size=inputs[-4].size()[-2:],
                          mode="bilinear",
                          align_corners=self.align_corners)

        # x = F.relu_(self.conv_4x(torch.cat([self.adapter_4x(inputs[-4]) + x, fusion],dim=1)))
        x = F.relu_(self.conv_4x(self.adapter_4x(inputs[-4]) + x))

        x = self.conv_out(x) + fusion
