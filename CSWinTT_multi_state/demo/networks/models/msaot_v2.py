...
from CSWinTT_multi_state.demo.networks.encoders import build_encoder
from CSWinTT_multi_state.demo.networks.layers.transformer import LongShortTermTransformer
from CSWinTT_multi_state.demo.networks.decoders import build_decoder
from CSWinTT_multi_state.demo.networks.layers.position import PositionEmbeddingSine
...
class AOT(nn.Module):
    def __init__(self, cfg, encoder='mobilenetv2', decoder='fpn'):
        ...
        if cfg.MODEL_ALIGN_CORNERS:
            self.patch_wise_id_bank = nn.Conv2d(
                ...
                stride=16,
                padding=8)
            self.patch_wise_id_bank_s8 = nn.Conv2d(
                cfg.MODEL_MAX_OBJ_NUM + 1,
                128,
                kernel_size=17,
                stride=16,
                padding=8)
            # self.patch_wise_id_bank_s4 = nn.Conv2d(
            #     cfg.MODEL_MAX_OBJ_NUM + 1,
            #     64,
            #     kernel_size=17,
            #     stride=16,
        ...
        self.pos_generator_s8 = PositionEmbeddingSine(
            128 // 2, normalize=True)


        self._init_weight()

    def get_pos_emb(self, x, scale=16):
        if scale==16:
            pos_emb = self.pos_generator(x)
        elif scale==8:
            pos_emb = self.pos_generator_s8(x)
        return pos_emb
    ...
    def LSTT_forward(self,
        ...
        size_2d_list.append((h,w))
        # 1/8
        n, c, h, w = curr_embs[-3].size()
        curr_emb_8 = curr_embs[-3]#.view(n, c, h * w).permute(2, 0, 1)
        size_2d_list.append((h, w))
        # 1/4
        n, c, h, w = curr_embs[-4].size()
        curr_emb_4 = curr_embs[-4]#.view(n, c, h * w).permute(2, 0, 1)
        size_2d_list.append((h, w))
        lstt_embs, lstt_memories = self.LSTT(curr_emb, curr_emb_8, curr_emb_4, long_term_memories,
                                             short_term_memories, curr_id_emb,
                                             pos_emb, size_2d_list)
