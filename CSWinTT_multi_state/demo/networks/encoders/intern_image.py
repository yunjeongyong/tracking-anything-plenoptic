...
class InternImage(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_features = int(channels * 2**(self.num_levels - 2)) 
        self.post_norm = post_norm
        self.mlp_ratio = mlp_ratio
        ...
        for i in range(self.num_levels):
            ...
            if i == 3:
                channel_temp=int(channels * 2**(i-1))
            else:
                channel_temp=int(channels * 2**i)
        ...
        # jiawen add load pretrained param
        # self.init_weights()
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            #         constant_init(m, 1.0)
            raise ValueError('without laod pretrained params')
        else:
            ...
            print('load pretrained backbone===============>', self.init_cfg['checkpoint'])
    ...
    def forward(self, x):
        ...
        for level_idx, level in enumerate(self.levels):
            if level_idx==3:
                ...
                input=x
            x, x_ = level(input, return_wo_downsample=True)
...
def INTERN_T():
    pretrained = './pretrain_models/internimage_t_1k_224.pth'
    print('use InternImage_T as backbone')
    model = InternImage(
        _delete_=True,
        type='InternImage',
        core_op='DCNv3',
...
def INTERN_XL():
    pretrained = None
    print('use InternImage_XL as backbone')
    model = InternImage(
        _delete_=True,
        type='InternImage',
        core_op='DCNv3',
...
def INTERN_H():
    pretrained = None
    print('use InternImage_H as backbone')
    model = InternImage(
        _delete_=True,
        type='InternImage',
        core_op='DCNv3',
