...
from CSWinTT_multi_state.demo.utils.utils.math import generate_permute_matrix
from CSWinTT_multi_state.demo.utils.utils.image import one_hot_mask

from CSWinTT_multi_state.demo.networks.layers.basic import seq_to_2d
...
class AOTEngine(nn.Module):
    ...
    def forward(self,
        ...
        # 一个视频会读进来5个样本，第一个是reference frame
        self.offline_encoder(all_frames, all_masks) # all_frames:[5*bs, 3, 465, 465],[5*bs, 1, 465, 465]
        # 传播第一张图
        self.add_reference_frame(frame_step=0, obj_nums=obj_nums)  # obj_nums: list, 该帧中存在的目标数目
    ...
    def encode_one_img_mask(self, img=None, mask=None, frame_step=-1):
        ...
        if self.enable_offline_enc:
            # 对于参考图，frame_step=0,选择第一张图的embs，有4个stage
            curr_enc_embs = self.offline_enc_embs[frame_step]
            # curr_enc_embs是一个tuple, 里面含有4张图的backbone feature, [bs,256,30,30], ref图像可能不在这里提取的emb
        elif img is None:
            curr_enc_embs = None
        else:
    ...
    def offline_encoder(self, all_frames, all_masks=None):
        ...
        # extract backbone features 提取了5张图的feature在list中，每张的的feature由tuple表示，内有4个stage的多尺度feature
        ...
        if all_masks is not None:
            ...
            # all_masks [bs*5, 1, 465, 465]
            # offline_one_hot_masks[10, 11, 465, 465]
    ...
    def assign_identity(self, one_hot_mask):
        ...
        # one_hot_mask=one_hot_mask.type(torch.HalfTensor).cuda()
        temps = self.AOT.get_id_emb(one_hot_mask)
        id_emb = temps[0].view(
            self.batch_size, -1, self.enc_hw).permute(2, 0, 1)
        # 换算一下其他尺度的feature size
        s8_size = [math.ceil(one_hot_mask.shape[-2]/8),math.ceil(one_hot_mask.shape[-1]/8)]
        id_emb_s8 = F.interpolate(temps[1],(s8_size[0], s8_size[1]),mode='nearest').view(
            self.batch_size, -1, s8_size[0] * s8_size[1]).permute(2, 0, 1)
        # id_emb_s4 = F.interpolate(temps[2], (117, 117), mode='nearest').view(
        #     self.batch_size, -1, 117*117).permute(2, 0, 1)
        # id_emb=id_emb
        id_emb=[id_emb, id_emb_s8]

        if self.training and self.freeze_id:
            id_emb = [id_emb[0].detach(), id_emb[1].detach()]
    ...
    def add_reference_frame(self,
        ...
        if self.pos_emb is None:
            # pos_emb: [900, 2, 256] [D, B, N] D=WxH=30X30
            # 这里的位置编码和proj后的encoder embs一致 128和64是我想把1/8和1/4 proj到这个维度
            pos_emb = self.AOT.get_pos_emb(curr_enc_embs[-1]).expand(
                self.batch_size, -1, -1,
                -1).view(self.batch_size, -1, self.enc_hw).permute(2, 0, 1)
            pos_emb_s8 = self.AOT.get_pos_emb(curr_enc_embs[-3], scale=8).expand(
                self.batch_size, -1, -1,
                -1).view(self.batch_size, -1, curr_enc_embs[-3].shape[-1]*curr_enc_embs[-3].shape[-2]
                         ).permute(2, 0, 1)
            # pos_emb_s4 = self.AOT.get_pos_emb(curr_enc_embs[-4][:, :64, :, :]).expand(
            #     self.batch_size, -1, -1,
            #     -1).view(self.batch_size, -1, curr_enc_embs[-4].shape[-1] * curr_enc_embs[-4].shape[-2]
            #              ).permute(2, 0, 1)
            self.pos_emb = [pos_emb, pos_emb_s8]
            # self.pos_emb = pos_emb
        # 产生 id emb: [bs, 11, 465, 465]=>[D, bs, N] [900,bs,256]的特征, id emb和pos emb一个维度
        curr_id_emb = self.assign_identity(curr_one_hot_mask) # list
        self.curr_id_embs = curr_id_emb

        # self matching and propagation
        # curr_enc_embs 含有4个stage的feature
        # self.enc_size_2d [30,30] 只用LSTT propgate 1/16的feature
    ...
    def update_long_term_memory(self, new_long_term_memories):
        ...
        #
        # ############## VOT debug
        # # f = open('/media/jiawen/File_JiaWenZ/Codes/VOT23/VOS_AOT/debug/debug.txt', 'a')
        # # print('%%%%%%%%%%%%%%%%%%%%%', file=f)
        # # print('new_long_term_memories', len(new_long_term_memories), file=f)
        # # print('new_long_term_memories[0]', len(new_long_term_memories[0]), file=f)
        # # print('new_long_term_memories[1]', len(new_long_term_memories[0][0]), file=f)
        # # f.close()
        # # import pdb
        # # pdb.set_trace()
        # ##############
        ...
        for new_long_term_memory, last_long_term_memory in zip(
            ...
            for new_e, last_e in zip(new_long_term_memory,
                ...
                if new_e is None or last_e is None:
                    ...
                    ############## VOT debug
                    # f = open('/media/jiawen/File_JiaWenZ/Codes/VOT23/VOS_AOT/debug/debug.txt', 'w')
                    # print('last_e', last_e.shape, file=f)
                    # f.close()
                    # import pdb
                    # pdb.set_trace()
                    ##############
                    if last_e.shape[0] >= MAX_LEN * TOKEN_NUM_dyna:
                        last_e = torch.cat([last_e[:(MAX_LEN - 2) * TOKEN_NUM_dyna],last_e[(MAX_LEN - 1) * TOKEN_NUM_dyna:]],dim=0)
                    updated_e.append(torch.cat([new_e, last_e], dim=0)) # 每中mem
            updated_long_term_memories.append(updated_e) # updated_e是一层的mem
        ############## VOT debug
        # f = open('/media/jiawen/File_JiaWenZ/Codes/VOT23/VOS_AOT/debug/debug.txt', 'a')
        # print('updated_long_term_memories', updated_long_term_memories[0][0].shape, file=f)
        # f.close()
        # import pdb
        # pdb.set_trace()
        ##############
        ...
        # f = open('/media/jiawen/File_JiaWenZ/Codes/VOT23/VOS_AOT/debug/debug.txt', 'a')
        # print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%',  file=f)
        # print('updated_long_term_memories', len(updated_long_term_memories), file=f)
        # print('updated_long_term_memories[0]', len(updated_long_term_memories[0]), file=f)
        # print('updated_long_term_memories[0]', updated_long_term_memories[0], file=f)
        # f.close()
        # import pdb
        # pdb.set_trace()
        ##############
        ...
        # print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
        # for i in range(len(updated_long_term_memories)): # 4层mem
        #     for j in range(len(updated_long_term_memories[i])): # 4中mem
        #         temp =updated_long_term_memories[i][j]
        #         if temp is not None:
        #             print('the {0} layer, the {1} class mem size: {2}'.format({i}, {j}, {temp.shape}))
